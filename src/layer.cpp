...
namespace {
	...
	void init_imgui_vulkan(
		...
		auto res = ImGui_ImplVulkan_LoadFunctions(
			...
			[](const char *name, void *user_data) -> PFN_vkVoidFunction {
				...
				PFN_vkGetDeviceProcAddr device_func = s.device_dispatch.read()->at(get_key(data->device)).vkGetDeviceProcAddr;
				PFN_vkGetInstanceProcAddr instance_func = s.instance_dispatch.read()->at(get_key(data->instance)).vkGetInstanceProcAddr;
				...
				s.l.trace("unknown function");
				s.l.flush();
				return nullptr;
			},
			&loaders
		);
		s.l.info("ImGui_ImplVulkan_LoadFunctions returned: {}", res);
	}
	...
	Q_CreateInstance(
		...
		s.instance_dispatch.mutate([&](auto &i_dld) {
			i_dld[get_key(instance)] = dispatchTable;
		});
		s.instance_map.mutate([&](auto &i_map) {
			i_map[get_key(instance)] = instance;
		});

		s.update_imgui_init_info([&](auto &info) -> void {
	...
	Q_CreateDevice(
		...
		vk::Instance instance = s.instance_map.read()->at(get_key(physicalDevice));
		vk::Device device{*pDevice};
		vk::detail::DispatchLoaderDynamic copy_dld = s.instance_dispatch.read()->at(get_key(physicalDevice));
		copy_dld.vkGetDeviceProcAddr = gdpa;
		copy_dld.init(device);
		s.device_dispatch.mutate([&](auto &d_dld) {
	...
	auto CALLBACK Q_HookedWndProc(
		...
		return CallWindowProcW(s.win32_hook.original_wndproc, hwnd, msg, wParam, lParam);
	}
	...
	Q_CreateWin32Surface(
		...
		PFN_vkCreateWin32SurfaceKHR func = s.instance_dispatch.read()->at(get_key(instance)).vkCreateWin32SurfaceKHR;

		HWND hwnd = pCreateInfo->hwnd;
		if (s.win32_hook.hwnd != hwnd) {
			ImGui_ImplWin32_Init(pCreateInfo->hwnd);
			s.win32_hook.hwnd = hwnd;
			s.win32_hook.original_wndproc = reinterpret_cast<WNDPROC>(
				SetWindowLongPtrW(hwnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(&Q_HookedWndProc))
			);

			s.l.trace("win32 wndproc subclassed for hwnd {}", static_cast<void *>(hwnd));
		}
	...
	Q_GetDeviceQueue(
		...
		PFN_vkGetDeviceQueue func = s.device_dispatch.read()->at(get_key(device)).vkGetDeviceQueue;

		func(device, queueFamilyIndex, queueIndex, pQueue);
		s.l.debug("Family: {} Queue: {}", queueFamilyIndex, queueIndex);
	...
	Q_GetDeviceQueue2(
		...
		PFN_vkGetDeviceQueue2 func = s.device_dispatch.read()->at(get_key(device)).vkGetDeviceQueue2;

		func(device, pQueueInfo, pQueue);
		s.l.debug("Family: {} Queue: {}", pQueueInfo->queueFamilyIndex, pQueueInfo->queueIndex);
	...
	Q_CreateRenderPass(
		...
		PFN_vkCreateRenderPass func = s.device_dispatch.read()->at(get_key(device)).vkCreateRenderPass;
	...
	Q_CreateSwapchain(
		...
		PFN_vkCreateSwapchainKHR func = s.device_dispatch.read()->at(get_key(device)).vkCreateSwapchainKHR;
		...
		s.update_imgui_init_info([&](ImGui_ImplVulkan_InitInfo &info) -> void {
			...
			auto pci = vk::PipelineRenderingCreateInfo(
				...
				&s.swapchain_color_format, // pColorAttachmentFormats
			...
			s.l.info("MinImageCount: {}", pCreateInfo->minImageCount);
		});
		return result;
	}
	...
	Q_DestroySwapchain(
		...
		PFN_vkDestroySwapchainKHR func = s.device_dispatch.read()->at(get_key(device)).vkDestroySwapchainKHR;

		s.remove_swapchain_data(swapchain);
	...
	Q_DestroyDevice(
		...
		PFN_vkDestroyDevice func = s.device_dispatch.read()->at(get_key(device)).vkDestroyDevice;
	...
	Q_QueueSubmit(
		...
		PFN_vkQueueSubmit func = s.device_dispatch.read()->at(get_key(queue)).vkQueueSubmit;
	...
	Q_QueuePresentKHR(
		...
		PFN_vkQueuePresentKHR func = s.device_dispatch.read()->at(get_key(queue)).vkQueuePresentKHR;

		auto result = func(queue, pPresentInfo);
		auto now = std::chrono::high_resolution_clock::now();
		auto elapsed = std::chrono::duration<double, std::milli>(now - s.last_frame);
		s.frametime = elapsed.count();
		s.last_frame = now;
	...
	Q_CmdEndRenderPass(
		...
		PFN_vkCmdEndRenderPass func = s.device_dispatch.read()->at(get_key(commandBuffer)).vkCmdEndRenderPass;
	...
	Q_CmdEndRenderPass2(
		...
		PFN_vkCmdEndRenderPass2 func = s.device_dispatch.read()->at(get_key(commandBuffer)).vkCmdEndRenderPass2;
...
extern "C" {
	...
	Q_GetDeviceProcAddr(VkDevice device, const char *pName) -> PFN_vkVoidFunction {
		...
		{
			PFN_vkGetDeviceProcAddr func = s.device_dispatch.read()->at(get_key(device)).vkGetDeviceProcAddr;
	...
	Q_GetInstanceProcAddr(VkInstance instance, const char *pName) -> PFN_vkVoidFunction {
		...
		{
			PFN_vkGetInstanceProcAddr func = s.instance_dispatch.read()->at(get_key(instance)).vkGetInstanceProcAddr;
